...
const AccountChatItem = (props: IAccountItem) => {
  ...
  return (
    <SCAccountItemWrapper>
      {props && (
        <List>
          <SCListItem onClick={() => handleTargetAccount(props)}>
            <ListItemAvatar>
            ...
            <SCListBodyItem>
              <SCAccountHeadItem style={{ flexDirection: 'column' }}>
                ...
                <SCSubName style={{ lineHeight: '1.2rem' }}>
                  {props.timestamp?.seconds && moment(props.timestamp?.seconds * 1000).format('LT')}
                </SCSubName>
              </SCAccountHeadItem>
            </SCListBodyItem>
          </SCListItem>
...
export default AccountChatItem;
